...
export const ProductCard = ({ product }: ProductCardProps) => {
  return (
    <Card className="p-4">  {/* если Card сам не добавляет padding */}
      ...
      <p className="text-gray-600 mt-2 line-clamp-2">{product.description}</p>  {/* обрезаем длинные описания */}
      <p className="text-lg font-bold mt-4 text-amber-400">{product.price} ₽</p>
      <Button size="md" className="mt-4 w-full">Add to cart</Button>  {/* можно full width */}
    </Card>
  );
}
