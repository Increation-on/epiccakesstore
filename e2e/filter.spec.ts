...
test('фильтр по категории "Торты" показывает только торты', async ({ page }) => {
  ...
  // 7. Проверяем, что активная категория подсвечена
  await expect(categoryButton).toHaveClass(/bg-\(--pink\)/);
  
  // 8. Проверяем, что товары есть (или хотя бы фильтр сработал)
  // Если в категории нет товаров — количество может быть 0, это нормально
